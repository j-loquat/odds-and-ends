...
# ---- Creating Dictionary of sample fuits data to model with ----
_dic = { 'Fruit Type': ['Blueberry', 'Avacado', 'Banana'], 'Quantity': [45, 28, 30]}
...
st.set_page_config(page_title="Streamlit Fruit Data Plot", page_icon=":bar_chart:", layout="wide")
...
if load or st.session_state.dataloaded:
    ...
    if plotopt == 'Bar':
        fig = px.bar(_df, x= 'Fruit Type', y = 'Quantity', title ='Fruit Bar Chart')
        plot_column.plotly_chart(fig)
    
    else:     
        fig = px.pie(_df, names = 'Fruit Type', values = 'Quantity', title ='Fruit Pie Chart')
        plot_column.plotly_chart(fig)


st.write("---")  # divider

# button to Reset - have to push it twice to work!
if data_column.button(label="Reset - Press x2", use_container_width=True):
